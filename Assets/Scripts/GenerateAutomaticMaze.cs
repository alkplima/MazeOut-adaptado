...
public class GenerateAutomaticMaze : Singleton<SaveHandler>
{
    ...
    private int avgHRBottomToTop, avgHRTopToBottom, avgHRLeftToRight, avgHRRightToLeft;
    ...
    private void ProcessAdaptationData()
    {
        ...
        switch (dataProcessingMode)
        {
            case 1:
                ...
                HeartRateFromPreviousMatchOnly();
                break;
            case 2:
    ...
    private int[] GetGridLimitsFromCalibrationCoordinates()
    {
        ...
        if (PlayerPrefs.GetInt("DataProcessingMode") == 1) // Deve usar HR
        {
            ...
            yMaxCellFinalIndex = IndexWithPseudoGrowthOrShrinkFromHR(yMaxCellIndex, avgHRBottomToTop, true, false, yMinCellIndex); // canto superior - usa bottomToTop
    ...
    {        
        ...
        maxX = VariaveisGlobais.maxX;
        maxY = VariaveisGlobais.maxY;
        avgHRBottomToTop = (!float.IsNaN(VariaveisGlobais.avgHRBottomToTop)) ? VariaveisGlobais.avgHRBottomToTop : 0;
        avgHRTopToBottom = (!float.IsNaN(VariaveisGlobais.avgHRTopToBottom)) ? VariaveisGlobais.avgHRTopToBottom : 0;
        avgHRLeftToRight = (!float.IsNaN(VariaveisGlobais.avgHRLeftToRight)) ? VariaveisGlobais.avgHRLeftToRight : 0;
        avgHRRightToLeft = (!float.IsNaN(VariaveisGlobais.avgHRRightToLeft)) ? VariaveisGlobais.avgHRRightToLeft : 0;
    }
    ...
    public int IndexWithPseudoGrowthOrShrinkFromHR(int index, float avgHRInDirection, bool shouldDecreaseToGrow, bool isColumn, int otherCoordinateExtremeIndex)
    {
        int cellGrowth;

        // Se HR reduzir, aumenta o labirinto
        if (avgHRInDirection < VariaveisGlobais.avgHRPartidaAnterior && avgHRInDirection != 0)
        {
            // Checa se o índice já está no limite do grid
            if (index == 0 || (isColumn && index==maxColumns - 1) || (!isColumn && index==maxRows - 1)) 
            {
            ...
            {
                // Se foi +- devagar ou se HR diminuiu (ficou mais calmo) -> não facilita, só mantém
                if (avgHRInDirection > VariaveisGlobais.avgHRPartidaAnterior)
                {
                    cellGrowth = 0;
                }
                else 
